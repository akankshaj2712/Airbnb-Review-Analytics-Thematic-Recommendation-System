...
RANKING (both search modes):
  score = ( (1 - distance)                                  # semantic similarity
...
def get_aspect_centroids():
    ...
    mat = np.vstack([emb.encode(seeds, normalize_embeddings=True).mean(axis=0)
...
def get_neighbourhoods():
    with get_conn().cursor() as cur:
        cur.execute("SELECT DISTINCT neighbourhood_cleansed FROM listings "
...
def get_room_types():
    with get_conn().cursor() as cur:
        cur.execute("SELECT DISTINCT room_type FROM listings "
...
def match_aspect(text):
    ...
    sims = mat @ qv
    i = int(sims.argmax())
    return (names[i], float(sims[i])) if sims[i] >= ASPECT_MATCH_THRESHOLD else (None, float(sims[i]))

# ------------------------------------------------------------------ data access
def _filters(neighbourhoods, price_max, room_types):
    where, params = ["e.embedding IS NOT NULL", f"COALESCE(l.number_of_reviews, 0) >= {MIN_REVIEWS}"], []
    if neighbourhoods:
        where.append("l.neighbourhood_cleansed = ANY(%s)"); params.append(neighbourhoods)
    if room_types:
        where.append("l.room_type = ANY(%s)"); params.append(room_types)
    if price_max:
        where.append("l.price <= %s"); params.append(price_max)
    return " AND ".join(where), params

def search(query_vec, neighbourhoods, price_max, room_types, k=TOP_K, aspect=None, alpha=0.0):
...
def render_card(row):
    with st.container(border=True):
        ...
        with c2:
            ...
            rating = f"{row['review_scores_rating']:.2f}" if pd.notnull(row.get("review_scores_rating")) else "—"
            st.caption(f"{row['neighbourhood_cleansed']} · {row['room_type']} · {price}/night · "
...
st.caption("Free-text semantic search · aspect-sentiment from ABSA · review-count weighted · pgvector")
...
with st.sidebar:
    ...
    st.divider()
    sentiment_aware = st.checkbox("Sentiment-aware ranking", value=True,
        help="If the query is about an aspect (e.g. cleanliness), boost listings praised for it.")
    alpha = st.slider("Sentiment weight", 0.0, 0.5, 0.20, 0.05, disabled=not sentiment_aware)
    st.caption(f"Listings with < {MIN_REVIEWS} reviews are excluded; "
...
if st.session_state.get("similar_to"):
    ...
    if st.button("← Back to search"):
        st.session_state.pop("similar_to", None); st.rerun()
    for _, row in similar_to(lid).iterrows():
        render_card(row.to_dict())
else:
    query = st.text_input("Describe what you want",
                          placeholder="e.g. clean quiet place near the water")
    if query:
        aspect, sim = (match_aspect(query) if sentiment_aware else (None, 0.0))
        with st.spinner("Searching..."):
            qvec = embed_query(query)
            df = search(qvec, nbhd, price_max, rooms,
                        aspect=aspect, alpha=(alpha if sentiment_aware else 0.0))
        if aspect:
            st.success(f"Detected aspect **{aspect}** — boosting listings guests praise for it "
                       f"(match {sim:.2f}). Toggle off for pure semantic ranking.")
        else:
            st.caption("No specific aspect detected — ranking by semantic similarity (review-weighted).")
        if df.empty:
